...
def hack2(request):
    ...
    # find all starting with a rack - put it on (rack, 1100), 
    dataOut = defaultdict(list)
    for key in result:
        src = key.split("->")[0]
        tgt = key.split("->")[1]
        if src != tgt:
            dataOut[src].extend(result[key])

    out = {}
    for key in dataOut:
        sum_values = defaultdict(int)
        for t, el in dataOut[key]:
            sum_values[t] += el
        out[key+"->all"] = sorted(sum_values.items())

    dataIn = defaultdict(list)
    # find all ending with a rack - put it on (1100, rack)
    for key in result:
        src = key.split("->")[0]
        tgt = key.split("->")[1]
        if src != tgt:
            dataIn[tgt].extend(result[key])

    ink = {}
    for key in dataIn:
        sum_values = defaultdict(int)
        for t, el in dataIn[key]:
            sum_values[t] += el
        ink["all->" + key] = sorted(sum_values.items())
